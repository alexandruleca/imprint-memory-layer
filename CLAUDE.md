...
- **Tagger**: structured payload `{lang, layer, kind, domain[], topics[]}`. Always-on deterministic + keyword dict; zero-shot on by default (opt-out `IMPRINT_ZERO_SHOT_TAGS=0`), now also used as fallback when LLM topics are empty; LLM tagging opt-in (`IMPRINT_LLM_TAGS=1`, replaces zero-shot). Context-aware: LLM tagger receives neighboring chunk text + project name for better topic accuracy. LLM providers: anthropic, openai, ollama, vllm, gemini — set via `IMPRINT_LLM_TAGGER_PROVIDER`. Default local model: Gemma 3 4B (Q4_K_M)
